...
namespace SocialPlus.Client
{
    ...
    /// </summary>
    public partial class MyPushRegistrations : IServiceOperations<SocialPlusClient>, IMyPushRegistrations
    {
        /// <summary>
        ...
        /// </param>
        public MyPushRegistrations(SocialPlusClient client)
        {
            if (client == null) 
            {
        ...
        public async Task<HttpOperationResponse<object>> PutPushRegistrationWithHttpMessagesAsync(Platform platform, string registrationId, PutPushRegistrationRequest request, string authorization, Dictionary<string, List<string>> customHeaders = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            var _url = new Uri(new Uri(_baseUrl + (_baseUrl.EndsWith("/") ? "" : "/")), "v0.8/users/me/push_registrations/{platform}/{registrationId}").ToString();
        ...
        /// A response object containing the response body and response headers.
        /// </return>
        public async Task<HttpOperationResponse<object>> DeletePushRegistrationWithHttpMessagesAsync(Platform platform, string registrationId, string authorization, Dictionary<string, List<string>> customHeaders = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (registrationId == null)
            {
                throw new ValidationException(ValidationRules.CannotBeNull, "registrationId");
            ...
            if (_shouldTrace)
            {
                ...
                tracingParameters.Add("authorization", authorization);
                tracingParameters.Add("cancellationToken", cancellationToken);
                ServiceClientTracing.Enter(_invocationId, this, "DeletePushRegistration", tracingParameters);
            }
            // Construct URL
            var _baseUrl = this.Client.BaseUri.AbsoluteUri;
            var _url = new Uri(new Uri(_baseUrl + (_baseUrl.EndsWith("/") ? "" : "/")), "v0.8/users/me/push_registrations/{platform}/{registrationId}").ToString();
